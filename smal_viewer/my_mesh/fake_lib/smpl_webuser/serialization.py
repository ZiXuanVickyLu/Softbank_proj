...
try:
    ...
    HAS_CHUMPY = False
    # Create a minimal chumpy replacement
    import numpy as np
...
def save_model(model, fname):
    ...
    # Changed 'w' mode to 'wb' for binary writing in Python 3
    with open(fname, 'wb') as f:
        pickle.dump(trainer_dict, f, protocol=pickle.HIGHEST_PROTOCOL)
...
def ready_arguments(fname_or_dict):
    if not isinstance(fname_or_dict, dict):
        # Changed to handle binary mode in Python 3
        with open(fname_or_dict, 'rb') as f:
            dd = pickle.load(f, encoding='latin1')  # Added encoding for Python 3 compatibility
    else:
        dd = fname_or_dict
